...
// 音声・チャット機能は voice-service.js に移動
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (startCamBtn) startCamBtn.onclick = window.startHomeworkWebcam;
});
...
window.startAnalysis = async function(b64) {
    ...
    let p = 0; 
    const timer = setInterval(() => { if (!window.isAnalyzing) { clearInterval(timer); return; } if (p < 30) p += 1; else if (p < 80) p += 0.4; else if (p < 95) p += 0.1; window.updateProgress(p); }, 300);
    const performAnalysisNarration = async () => {
        ...
        for (const item of msgs) { if (!window.isAnalyzing) return; await window.updateNellMessage(item.text, item.mood, false); if (!window.isAnalyzing) return; await new Promise(r => setTimeout(r, 1500)); }
    };
    performAnalysisNarration();
    try {
        const res = await fetch('/analyze', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ image: b64, mode: window.currentMode, grade: currentUser.grade, subject: window.currentSubject, name: currentUser.name }) });
        ...
        window.isAnalyzing = false; clearInterval(timer); window.updateProgress(100); window.cleanupAnalysis();
        
        if(window.safePlay) window.safePlay(window.sfxHirameku);
        
        setTimeout(() => { document.getElementById('thinking-view').classList.add('hidden'); const doneMsg = "読めたにゃ！"; if (window.currentMode === 'grade') { window.showGradingView(true); window.updateNellMessage(doneMsg, "happy", false).then(() => setTimeout(window.updateGradingMessage, 1500)); } else { window.renderProblemSelection(); window.updateNellMessage(doneMsg, "happy", false); } }, 1500); 
    } catch (err) { 
        console.error("Analysis Error:", err); window.isAnalyzing = false; window.cleanupAnalysis(); clearInterval(timer); 
