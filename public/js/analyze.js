...
window.saveToNellMemory = function(role, text) {
    ...
    try {
        ...
        if (history.length > 0 && history.text === trimmed) return;
...
window.renderHintUI = function() {
    ...
    if (maxUnlocked > 0) {
        ...
        for (let i = 1; i <= maxUnlocked; i++) { 
            const btn = document.createElement('button'); btn.className = "main-btn gray-btn"; btn.style.fontSize = "0.9rem"; btn.style.padding = "8px"; btn.style.flex = "1"; btn.innerText = `ヒント${i}を見る`; btn.onclick = () => window.showHintText(i); reviewContainer.appendChild(btn); 
        }
...
window.revealAnswer = function() {
    ...
    const correctArr = Array.isArray(window.selectedProblem.correct_answer) ? window.selectedProblem.correct_answer :;
    let displayAnswer = correctArr.map(part => part.split('|')).join(', ');
...
function createProblemItem(p, mode) {
    ...
    let correctList = Array.isArray(p.correct_answer) ? p.correct_answer :;
    correctList = correctList.map(s => String(s).trim()).filter(s => s !== ""); 
    let studentList = Array.isArray(p.student_answer) ? p.student_answer :;
        ...
        if (isCorrect === undefined) { 
            if (correctList.length !== studentList.length) isCorrect = false; 
                ...
                for(let i=0; i<correctList.length; i++) { 
                    if (!window.isMatch(studentList || "", correctList)) { isCorrect = false; break; } 
                } 
            } 
        }
...
window._performCheckMultiAnswer = function(id) {
    ...
    const userValues = problem.student_answer; const correctList = Array.isArray(problem.correct_answer) ? problem.correct_answer :;
        ...
        for (const uVal of userValues) { 
            for (let i = 0; i < correctList.length; i++) { 
                if (!usedIndices.has(i)) { 
                    if (window.isMatch(uVal, correctList)) { usedIndices.add(i); matchCount++; break; } 
                } 
            } 
        } 
...
window.checkAnswerDynamically = function(id, inputElem, event) { 
    if (window.isComposing) return; const problem = window.transcribedProblems.find(p => p.id === id); if(problem) problem.student_answer =; const val = inputElem.value;
...
window._performCheckAnswerDynamically = function(id, val) {
    const problem = window.transcribedProblems.find(p => p.id === id); if (!problem) return;
    const correctVal = Array.isArray(problem.correct_answer) ? problem.correct_answer : problem.correct_answer;
...
window.initCustomCropper = function() { 
    ...
    const move = (e) => { 
        ...
        window.cropPoints = { x: relX * canvas.width, y: relY * canvas.height }; 
        window.updateCropUI(canvas); 
    }; 
...
window.updateCropUI = function(canvas) { 
    ...
    screenPoints.forEach((p, i) => { 
        const el = document.getElementById(handles); 
    ...
    if(svg) { 
        ...
        svg.innerHTML = `<polyline points="${ptsStr} ${screenPoints.x},${screenPoints.y}" style="fill:rgba(255,255,255,0.2);stroke:#ff4081;stroke-width:2;stroke-dasharray:5" />`; 
    } 
};
...
window.performPerspectiveCrop = function(sourceCanvas, points) { 
    ...
    const result = window.processImageForAI(tempCv).split(','); 
    return result; 
};
...
window.startAnalysis = async function(b64) {
    ...
    try { 
        ...
        window.transcribedProblems = data.map((prob, index) => { 
            let studentArr = Array.isArray(prob.student_answer) ? prob.student_answer : (prob.student_answer ? :[]); 
            let correctArr = Array.isArray(prob.correct_answer) ? prob.correct_answer : (prob.correct_answer ? :[]); 
            return { ...prob, id: index + 1, student_answer: studentArr, correct_answer: correctArr, status: (studentArr.length > 0 && studentArr !== "") ? "answered" : "unanswered", currentHintLevel: 1, maxUnlockedHintLevel: 0 }; 
        }); 
...
window.captureAndSendLiveImage = function(context = 'main') {
    ...
    const compressedDataUrl = window.processImageForAI(canvas); const base64Data = compressedDataUrl.split(',');
...
window.handleChatImageFile = async function(file, context = 'embedded') {
    ...
    reader.onload = async (e) => {
        ...
        img.onload = async () => {
            ...
            const compressedDataUrl = canvas.toDataURL('image/jpeg', 0.6); const base64Data = compressedDataUrl.split(',');
