// --- js/game-engine.js (完全版 v383.0: VS掃除機画像差し替え・ライフ制版) ---
...
function loadDanmakuImages() {
    ...
    danmakuImages.player.src = DANMAKU_ASSETS_PATH + 'neru_dot.png';
    danmakuImages.boss.src = DANMAKU_ASSETS_PATH + 'runba_dot.png';
    
    // Goodアイテム読み込み
    danmakuImages.goods = goodItemsDef.map(def => {
        const img = new Image();
        img.src = DANMAKU_ASSETS_PATH + def.file;
    ...
    danmakuImages.bads = badItemsDef.map(file => {
        const img = new Image();
        img.src = DANMAKU_ASSETS_PATH + file;
        return img;
    });
