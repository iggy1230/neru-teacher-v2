// --- js/game-engine.js (完全版 v394.0: 文法エラー修正・展開版) ---
...
window.nextRiddle = async function() { 
    ...
    if (riddleData && riddleData.question) { 
        ...
        fetchRiddleData().then(data => { 
            riddleState.nextRiddle = data; 
        }).catch(err => console.warn("Pre-fetch failed", err)); 
    } else { 
        qText.innerText = "エラーだにゃ…"; 
    } 
...
window.clearKanjiCanvas = function() { 
    ...
    kanjiState.ctx.beginPath(); 
    kanjiState.ctx.moveTo(150, 0); 
    kanjiState.ctx.lineTo(150, 300); 
    kanjiState.ctx.moveTo(0, 150); 
    kanjiState.ctx.lineTo(300, 150); 
    kanjiState.ctx.stroke(); 
