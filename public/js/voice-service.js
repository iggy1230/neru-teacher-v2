// --- js/voice-service.js (完全版 v397.0: 安定化・ノイズフィルタ搭載版) ---
...
window.startMicrophone = async function() { 
    ...
    try { 
        ...
        window.workletNode.port.onmessage = (event) => { 
            ...
            // ★改善: フラグガード + 自動タイムアウト安全装置
            if (window.isNellSpeaking) {
                 // 万が一フラグが降りない場合のための安全装置 (3秒以上話してることになってたら強制解除)
                 if (!window.speechSafetyTimer) {
                     window.speechSafetyTimer = setTimeout(() => {
                         console.log("Force resetting speech flag");
                         window.isNellSpeaking = false;
                         window.speechSafetyTimer = null;
                     }, 3000);
                 }
                 return;
            } else {
                if (window.speechSafetyTimer) { clearTimeout(window.speechSafetyTimer); window.speechSafetyTimer = null; }
            }
