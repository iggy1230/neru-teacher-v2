// --- js/voice-service.js (v303.0: リアルタイム音声安定化・エラー対策版) ---
// 音声再生の停止
window.stopAudioPlayback = function() {
// 既存のソースを停止
if (window.liveAudioSources) {
window.liveAudioSources.forEach(source => {
try { source.stop(); } catch(e){}
});
}
window.liveAudioSources = [];
// 次の再生開始位置をリセット（現在時刻より少し先へ）
if (window.audioContext && window.audioContext.state === 'running') {
    window.nextStartTime = window.audioContext.currentTime + 0.1;
}
...
window.continuousRecognition.onresult = async (event) => {
    // ネル先生が喋っている最中は、マイク入力を無視する（エコーバック対策）
    if (window.isNellSpeaking) {
         console.log("[Ignored User Speech] Nell is speaking.");
         return;
    }
...
// ==========================================
// WebSocketチャット (リアルタイム)
// ==========================================
// 画像送信 (WebSocket / HTTP 共用)
...
window.isActiveChatSession = false; // フラグで処理をガード
...
if (window.workletNode) { 
    try {
        window.workletNode.port.postMessage('stop'); 
        window.workletNode.disconnect(); 
    } catch(e){}
} 

if (window.liveSocket) {
    try { window.liveSocket.close(); } catch(e){}
    window.liveSocket = null;
}

// AudioContext の破棄 (エラー防止のためステート確認)
if (window.audioContext) {
    if (window.audioContext.state !== 'closed') {
        window.audioContext.close().catch(e => console.warn("AC Close Error:", e));
    }
...
window.isActiveChatSession = true; // セッション有効フラグ
...
try { 
    ...
    // 古い AudioContext があれば確実に閉じる
    if (window.audioContext) {
        if(window.audioContext.state !== 'closed') await window.audioContext.close();
        window.audioContext = null;
    }
    ...
    // GainNodeを作り直して window.ttsGainNode に再代入
    ...
    window.liveSocket.onopen = () => { 
        if (!window.isActiveChatSession) { window.liveSocket.close(); return; }
        window.liveSocket.send(JSON.stringify({
            type: "init",
    ...
    window.liveSocket.onmessage = async (event) => { 
        if (!window.isActiveChatSession) return; // セッション終了後は無視
    ...
    window.liveSocket.onclose = () => { if(window.isActiveChatSession) window.stopLiveChat(); }; 
    window.liveSocket.onerror = () => { if(window.isActiveChatSession) window.stopLiveChat(); }; 
} catch (e) { window.stopLiveChat(); }
};
window.startMicrophone = async function() {
try {
// 1. Web Speech API (音声認識) の設定
if ('webkitSpeechRecognition' in window) {
...
window.recognition.onresult = (event) => { 
            if (!window.isActiveChatSession) return;
            ...
            // 自分が喋っている最中の認識結果は、エコーキャンセルのため無視
            if (window.isNellSpeaking) {
                 // 明確な停止コマンド以外は無視
                 const stopKeywords = ["違う", "ちがう", "待って", "まって", "ストップ", "やめて"];
    ...
    if (window.audioContext && window.audioContext.state !== 'closed') {
        ...
        window.workletNode.port.onmessage = (event) => { 
            if (!window.isActiveChatSession || window.isMicMuted) return;
            if (!window.liveSocket || window.liveSocket.readyState !== WebSocket.OPEN) return; 
            
            // ネル先生が喋っている間は、音声を送信しない（エコーキャンセル簡易版）
...
// セッション終了後、または音声無視フラグ時は再生しない
if (!window.isActiveChatSession || window.ignoreIncomingAudio) return;

// コンテキストチェック: 閉じていたら何もしない
if (!window.audioContext || window.audioContext.state === 'closed') return;
...
try {
    ...
    // GainNodeを使用 (エラー回避のため destination フォールバック)
    ...
    if (window.nextStartTime < now || window.nextStartTime > now + 5.0) {
    ...
    // 再生開始少し前にフラグを立てる
    window.speakingStartTimer = setTimeout(() => { window.isNellSpeaking = true; }, Math.max(0, startDelay - 50)); 
    ...
    console.warn("PCM Playback Error (Ignored):", e);
}
};
