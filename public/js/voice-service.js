// --- js/voice-service.js (完全版 v396.0: iPhone完全対応・マイク初期化修正版) ---
...
window.startAlwaysOnListening = function() {
    // iPhone等のWebSpeechAPI非対応端末では何もしない（エラー防止）
    ...
    try {
        ...
        window.continuousRecognition.onresult = async (event) => {
            ...
            if (window.isNellSpeaking) {
                ...
                if (shouldInterrupt(textToCheck)) {
                    console.log("[Interruption]", textToCheck);
        ...
        window.continuousRecognition.start();
    } catch(e) { console.log("Rec start failed", e); }
};
...
window.stopLiveChat = function() {
    if (window.NellMemory && window.chatTranscript && window.chatTranscript.length > 10 && typeof window.currentUser !== 'undefined') {
    ...
    if (window.audioContext && window.audioContext.state !== 'closed') {
        // AudioContextは閉じずにsuspendする（再利用のため）か、閉じる
        window.audioContext.close().catch(e => console.warn(e));
...
window.startLiveChat = async function(context = 'main') { 
    ...
    // ★重要: iPhone対策。ボタンクリックのタイミングでAudioContextを作成・再開する
    ...
    try { 
        ...
        let memoryContext = "";
        if (window.NellMemory) memoryContext = await window.NellMemory.generateContextString(window.currentUser.id);
...
window.startMicrophone = async function() { 
    window.isNellSpeaking = false; // フラグリセット
    
    try { 
        const useVideo = (window.currentMode !== 'chat-free');
        
        // ★修正: iPhone互換性のため sampleRate 指定を外す (OSに任せる)
        ...
        try {
             await window.audioContext.audioWorklet.addModule(blobUrl); 
        } catch (e) {
             // すでに追加されている場合は無視
        }
        ...
        window.workletNode.port.onmessage = (event) => { 
            ...
            // ★修正: AudioContextの実際のレートを使ってダウンサンプリング
        ...
        alert("マイクが使えないにゃ。ブラウザの設定でマイクを許可してにゃ！");
    } 
};
