// --- js/ui/ui.js (v294.0: 完全版 - メッセージ・ログ機能統合) ---
// 音声ファイルのパス設定（ローカル定義）
...
if (window.safePlay) window.safePlay(sfxChime);
window.switchScreen('screen-gate');
};
window.backToTitle = async function() {
if (typeof window.logoutProcess === 'function') {
await window.logoutProcess();
}
window.switchScreen('screen-title');
};
...
if (shouldGreet && typeof currentUser !== 'undefined' && currentUser) {
    window.updateNellMessage(`おかえり、${currentUser.name}さん！`, "happy");
}
...
// 表示用テキストの整形
let displayText = t ? t.replace(/(?:\[|\【)?DISPLAY[:：]\s*(.+?)(?:\]|\】)?/gi, "") : "";

if (el) el.innerText = displayText;

// もぐもぐ演出 (constants.jsのAudioを使用)
if (t && t.includes("もぐもぐ") && window.sfxBori) { 
    if(window.safePlay) window.safePlay(window.sfxBori); 
}

// 記憶への保存
if (saveToMemory && typeof window.saveToNellMemory === 'function') { 
    window.saveToNellMemory('nell', t); 
}
...
window.switchScreen('screen-attendance');
...
if (modal) {
modal.classList.remove('hidden');
window.switchMemoryTab('profile');
}
};
...
if(window.safePlay) window.safePlay(sfxBtn);
}
}
});