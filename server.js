...
app.post('/identify-item', async (req, res) => {
    try {
        ...
        // ★修正: 座標(EXIF)がある場合はそれを最優先し、現在地(address)は無視する
        if (location && location.lat && location.lon) {
            ...
            **【★絶対厳守：場所の特定手順】**
            1. **まずGoogle検索で「${location.lat},${location.lon} 住所」を検索し、ピンポイントな住所（市町村名＋スポット名）を特定してください。**
            2. **たとえユーザーから現在地の住所（${address || '不明'}）が提供されていたとしても、それは無視してください。** 画像内の物体は、あくまで上記の「GPS座標」にあるものとして特定・解説を行ってください。
            3. 画像の見た目が有名な観光地や別の場所に似ていても、**GPS座標が示す場所にある施設や公園**として特定してください。
            `;
        } else if (address) {
            // EXIFがなく、現在地住所のみの場合
            locationInfo = `
            ...
            指示:
            1. ユーザーは「${address}」にいます。
            2. この住所周辺にある、画像に写っているようなものをGoogle検索で特定してください。
            3. 住所の細かい部分（大字、字、番地など）は省略し、「都道府県＋市町村名」＋「スポット名」で呼んでください。
            `;
        }
        ...
        **相手を呼ぶときは必ず「${name}さん」と呼んでください。絶対に呼び捨てにしてはいけません。**
        ...
        4. **場所の言及ルール**: 
           - **EXIF座標がある場合**: 特定された場所（市町村名＋スポット名）を正確に伝えてください（例: 「ここは〇〇市の××公園だにゃ！」）。
