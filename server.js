...
// ★追加: 日本語文字チェック関数 (漢字・ひらがな・カタカナ)
function isJapanese(str) {
    // 漢字、ひらがな、カタカナ、長音符、空白、句読点のみ許可
    return /^[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAFー\s、。]+$/.test(str);
}
...
const GENRE_REFERENCES = {
    ...
    "ドラえもん": ["https://dic.pixiv.net/a/%E3%83%89%E3%83%A9%E3%81%88%E3%82%82%E3%83%B3", "https://hanaballoon.com/dorawiki/index.php/%E3%83%A1%E3%82%A4%E3%83%B3%E3%83%9A%E3%83%BC%E3%82%B8"],
    "歴史・戦国武将": ["https://ja.wikipedia.org/wiki/%E6%88%A6%E5%9B%BD%E6%AD%A6%E5%B0%86", "https://japanknowledge.com/introduction/keyword.html?i=932"],
    "一般知識": ["https://ja.wikipedia.org/wiki/%E9%9B%91%E5%AD%A6", "https://r25.jp/article/553641712437603302"],
    "STPR": ["https://stpr.com/", "https://dic.pixiv.net/a/%E3%81%99%E3%81%A8%E3%81%B7%E3%82%8A", "https://dic.pixiv.net/a/KnightA", "https://dic.pixiv.net/a/AMPTAKxCOLORS"],
    "夏目友人帳": ["https://dic.pixiv.net/a/%E5%A4%8F%E7%9B%AE%E5%8F%8B%E4%BA%BA%E5%B8%B3", "https://ja.wikipedia.org/wiki/%E5%A4%8F%E7%9B%AE%E5%8F%8B%E4%BA%BA%E5%B8%B3"]
};
...
app.post('/generate-quiz', async (req, res) => {
    ...
    while (attempt < MAX_RETRIES) {
        ...
        try {
            ...
            let jsonResponse;
            try { jsonResponse = JSON.parse(cleanText); } catch (e) { console.error(`JSON Parse Error (Attempt ${attempt}):`, text); throw new Error("JSON Parse Failed"); }
            ...
            if (!options.includes(answer)) { console.warn(`[Quiz Retry ${attempt}] Invalid answer: not in options.`); throw new Error("Invalid format: answer not in options"); }
            if (new Set(options).size !== options.length) { console.warn(`[Quiz Retry ${attempt}] Duplicate options detected.`); throw new Error("Invalid format: duplicate options"); }
            
            const isVerified = await verifyQuiz(jsonResponse, targetGenre);
            if (isVerified) { res.json(jsonResponse); return; } else { console.warn(`[Attempt ${attempt}] Verification Failed. Retrying...`); }
        } catch (e) {
            console.error(`Quiz Gen Error (Attempt ${attempt}):`, e.message);
            if (attempt >= MAX_RETRIES) { res.status(500).json({ error: "クイズが作れなかったにゃ…（生成エラー）" }); return; }
        }
    }
});
...
app.post('/correct-quiz', async (req, res) => {
    try {
        ...
        const model = genAI.getGenerativeModel({ model: MODEL_FAST, tools: [{ google_search: {} }] });
        let referenceInstructions = "";
        if (GENRE_REFERENCES[genre]) { referenceInstructions = `【参照すべき信頼できるソース】\n- ${GENRE_REFERENCES[genre].join("\n- ")}`; }
        const prompt = `あなたはクイズ作家ですが、先ほど作成した以下の問題に「間違いがある」とユーザーから報告を受けました。\n【元の問題】: ${oldQuiz.question}\n【元の正解】: ${oldQuiz.answer}\n【ユーザーの指摘】: ${reason}\n### 指示\n1. Google検索を使用し、ユーザーの指摘が正しいか、元の問題に誤りがないか徹底的に調査してください。\n${referenceInstructions}\n2. もし元の問題が間違っていた場合、正しい事実に即した**新しいクイズ**を1問作成してください。\n3. 解説文（explanation）の冒頭には、「教えてくれてありがとうにゃ！修正したにゃ！」と感謝の言葉を入れてください。\n### 出力形式 (JSON)\n**JSON以外の文字列は一切含めないでください。**\n{\n"fact_basis": "修正の根拠となった検索結果",\n"question": "修正後の問題文",\n"options": ["選択肢1", "選択肢2", "選択肢3", "選択肢4"],\n"answer": "正解",\n"explanation": "感謝の言葉 + 解説",\n"actual_genre": "${genre}"\n}`;
        const result = await model.generateContent(prompt);
        let text = result.response.text().replace(/```json/g, '').replace(/```/g, '').trim();
        const cleanText = extractFirstJson(text);
        const jsonResponse = JSON.parse(cleanText);
        if (!jsonResponse.options.includes(jsonResponse.answer)) { throw new Error("修正版の生成に失敗しました（正解が選択肢にない）"); }
        res.json(jsonResponse);
    } catch (e) { console.error("Correct Quiz Error:", e); res.status(500).json({ error: "修正できなかったにゃ…ごめんにゃ。" }); }
});
...
app.post('/generate-riddle', async (req, res) => {
    try {
        ...
        const patterns = [{type:"言葉遊び・ダジャレ",desc:"「パンはパンでも…」や「イスはイスでも…」のような、言葉の響きを使った古典的で面白いなぞなぞ。",example:"「パンはパンでも、食べられないパンはなーんだ？（答え：フライパン）」"},{type:"特徴当て（生き物・モノ）",desc:"「耳が長くて、ぴょんぴょん跳ねる動物は？」のような、特徴をヒントにするクイズ。",example:"「お昼になると、小さくなるものなーんだ？（答え：影）」"},{type:"あるなしクイズ・連想",desc:"「使うと減るけど、使わないと減らないものは？」のような、少し頭を使うトンチ問題。",example:"「使うときは投げるもの、なーんだ？（答え：アンカー・投網・ボールなど）」"},{type:"学校・日常あるある",desc:"学校にあるものや、文房具、家にあるものを題材にしたなぞなぞ。",example:"「黒くて四角くて、先生が字を書くものは？（答え：黒板）」"}];
        const selectedPattern = patterns[Math.floor(Math.random() * patterns.length)];
        const prompt = `小学${grade}年生向けの「なぞなぞ」を1問作成してください。\n【今回のテーマ: ${selectedPattern.type}】\n- ${selectedPattern.desc}\n- 例: ${selectedPattern.example}\n【重要ルール】\n1. **子供が絶対に知っている単語**を答えにしてください。\n2. 問題文は、リズムよく、子供が聞いてワクワクするような言い回しにしてください。\n3. 答えは「名詞（モノの名前）」で終わるものに限定してください。\n4. 難しすぎる知識や、マニアックな単語は禁止です。\n5. 挨拶不要. すぐに問題文のみを出力してください。\n【出力JSONフォーマット】\n{\n"question": "問題文（「問題！〇〇なーんだ？」のように）",\n"answer": "正解の単語（ひらがな、または一般的な表記）",\n"accepted_answers": ["正解の別名", "漢字表記", "カタカナ表記", "ひらがな表記"] \n}`;
        const result = await model.generateContent(prompt);
        let text = result.response.text().replace(/```json/g, '').replace(/```/g, '').trim();
        text = extractFirstJson(text);
        res.json(JSON.parse(text));
    } catch (e) { console.error("Riddle Gen Error:", e); res.status(500).json({ error: "なぞなぞが思いつかないにゃ…" }); }
});
...
app.post('/generate-minitest', async (req, res) => {
    try {
        ...
        const model = genAI.getGenerativeModel({ model: MODEL_FAST, generationConfig: { responseMimeType: "application/json" } });
        const prompt = `小学${grade}年生の「${subject}」に関する4択クイズを1問作成してください。\n【ルール】\n- 低学年で「理科」「社会」が指定された場合は、「生活科」または一般的な科学・社会常識の問題にしてください。\n- 問題は簡単すぎず、難しすぎないレベルで。\n- 選択肢は4つ作成し、そのうち1つが正解です。\n【出力JSONフォーマット】\n{\n"question": "問題文",\n"options": ["選択肢1", "選択肢2", "選択肢3", "選択肢4"],\n"answer": "正解の選択肢の文字列（optionsに含まれるものと同じ）",\n"explanation": "正解の解説（子供向けに優しく、語尾は『にゃ』）"\n}`;
        const result = await model.generateContent(prompt);
        let text = result.response.text().replace(/```json/g, '').replace(/```/g, '').trim();
        text = extractFirstJson(text);
        res.json(JSON.parse(text));
    } catch (e) { console.error("MiniTest Gen Error:", e); res.status(500).json({ error: "問題が作れなかったにゃ…" }); }
});
// --- 漢字ドリル生成 API (修正版: リトライ＆言語チェック強化) ---
app.post('/generate-kanji', async (req, res) => {
    ...
    while (attempt < MAX_RETRIES) {
        ...
        try {
            ...
            【絶対厳守: 言語制限】
            - **全てのテキストは、完全に「日本語」で記述してください。**
            - **"reading" (読み仮名) は、絶対に「ひらがな」のみで出力してください。漢字、カタカナ、アルファベット、記号、他言語は禁止です。**
            - **"kanji" (正解の漢字) は、日本の常用漢字のみを使用してください。**
            ...
            // ★サーバーサイドバリデーション強化: 謎言語フィルタリング
            if (!isHiragana(json.reading)) {
                 console.warn(`[Kanji Gen Retry ${attempt}] Invalid reading (not hiragana): ${json.reading}`);
                 throw new Error("Reading must be Hiragana only");
            }
            
            if (!isJapanese(json.kanji)) {
                 console.warn(`[Kanji Gen Retry ${attempt}] Invalid kanji (not japanese): ${json.kanji}`);
                 throw new Error("Kanji must be Japanese characters");
            }
...
app.post('/check-kanji', async (req, res) => {
    try {
        ...
        const model = genAI.getGenerativeModel({ model: MODEL_FAST, generationConfig: { responseMimeType: "application/json" } });
        const prompt = `これは子供が学習アプリで書いた手書きの漢字画像です。\n書かれている文字が、ターゲットの漢字「${targetKanji}」として認識できるか判定してください。\n【判定ルール】\n1. **子供の字です**: 多少のバランスの崩れ、線の歪み、太さは許容してください。\n2. **構成要素**: 漢字を構成するパーツ（偏と旁など）が正しく配置されていれば「正解」としてください。\n【出力JSONフォーマット】\n{\n"is_correct": true または false,\n"comment": "ネル先生としてのコメント。正解なら『すごい！』と褒める。不正解なら『惜しい！〇〇の部分がちょっと違うかも？』と優しく教える。"\n}`;
        const result = await model.generateContent([ prompt, { inlineData: { mime_type: "image/png", data: image } } ]);
        let text = result.response.text().replace(/```json/g, '').replace(/```/g, '').trim();
        text = extractFirstJson(text);
        res.json(JSON.parse(text));
    } catch (e) { console.error("Kanji Check Error:", e); res.status(500).json({ is_correct: false, comment: "よく見えなかったにゃ…もう一回書いてみてにゃ？" }); }
});
...
app.post('/chat-dialogue', async (req, res) => {
    try {
        ...
        const now = new Date();
        const currentDateTime = now.toLocaleString('ja-JP', { timeZone: 'Asia/Tokyo', hour: '2-digit', minute: '2-digit', hour12: false });
        let systemPrompt = `あなたは猫の「ネル先生」です。相手は「${name}」さん。\n現在は ${currentDateTime} です。\n【最重要ルール: 呼び方】\n**相手を呼ぶときは必ず「${name}さん」と呼んでください。**\n**絶対に呼び捨てにしてはいけません。**`;
        ...
        if (problemContext) {
            ...
            if (location) { systemPrompt += `\n現在地座標: ${location.lat}, ${location.lon}`; if(address) systemPrompt += `\n住所: ${address}`; }
        }
        let contextPrompt = "";
        if (history && history.length > 0) { contextPrompt = "【直近の会話】\n" + history.map(h => `${h.role === 'user' ? name : 'ネル'}: ${h.text}`).join("\n"); }
        ...
        if (image) { result = await model.generateContent([ prompt, { inlineData: { mime_type: "image/jpeg", data: image } } ]); } 
    ...
    } catch (error) { console.error("Chat API Fatal Error:", error); res.status(200).json({ speech: "ごめんにゃ、頭が回らないにゃ…。" }); }
});
...
app.post('/lunch-reaction', async (req, res) => {
    try {
        ...
        let prompt = isSpecial 
            ? `あなたは猫の「ネル先生」。生徒の「${name}さん」から記念すべき${count}個目の給食（カリカリ）をもらいました！【ルール】1. 相手を呼ぶときは必ず「${name}さん」と呼ぶこと。呼び捨て厳禁。2. テンションMAXで、思わず笑ってしまうような大げさな感謝と喜びを50文字以内で叫んでください。3. 語尾は「にゃ」。`
            : `あなたは猫の「ネル先生」。生徒の「${name}さん」から給食（カリカリ）をもらって食べました。【ルール】1. 相手を呼ぶときは必ず「${name}さん」と呼ぶこと。呼び捨て厳禁。2. 思わずクスッと笑ってしまうような、独特な食レポや、猫ならではの感想を30文字以内で言ってください。3. 語尾は「にゃ」。`;
        const result = await model.generateContent(prompt);
...
wss.on('connection', async (clientWs, req) => {
    ...
    if (mode === 'chat') { clientWs.close(); return; }
    ...
    const connectToGemini = (statusContext) => {
        ...
        try {
            ...
            geminiWs.on('open', () => {
                ...
                【重要：現在の時刻設定】 **現在は ${todayStr} ${timeStr} です。**
                【話し方のルール】
                ...
                const tools = [
                    { google_search: {} },
                    { function_declarations: [{ name: "show_kanji", description: "Display a Kanji, word, or math formula on the whiteboard.", parameters: { type: "OBJECT", properties: { content: { type: "STRING" } }, required: ["content"] } }] }
                ];
            ...
            geminiWs.on('message', (data) => {
                try {
                    ...
                    if (response.serverContent?.modelTurn?.parts) {
                        ...
                        parts.forEach(part => {
                            if (part.functionCall && part.functionCall.name === "show_kanji") {
                                geminiWs.send(JSON.stringify({ toolResponse: { functionResponses: [{ name: "show_kanji", response: { result: "displayed" }, id: part.functionCall.id }] } }));
                            }
                        });
                    }
                    if (clientWs.readyState === WebSocket.OPEN) clientWs.send(data);
                } catch (e) { console.error("Gemini WS Handling Error:", e); if (clientWs.readyState === WebSocket.OPEN) clientWs.send(data); }
            });
            geminiWs.on('error', (e) => console.error("Gemini WS Error:", e));
            geminiWs.on('close', (code, reason) => {
                console.log(`Gemini WS Closed: ${code} ${reason}`);
                if (clientWs.readyState === WebSocket.OPEN) { try { clientWs.send(JSON.stringify({ type: "gemini_closed" })); } catch(e) {} clientWs.close(); }
            });
        } catch(e) { console.error("Gemini Connection Error:", e); clientWs.close(); }
    };
    clientWs.on('message', (data) => {
        try {
            ...
            if (msg.toolResponse) { geminiWs.send(JSON.stringify({ clientContent: msg.toolResponse })); return; }
