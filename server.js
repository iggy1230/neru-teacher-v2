// --- server.js (完全版 v326.0: 図鑑レアリティ判定実装版) ---
...
app.post('/identify-item', async (req, res) => {
    try {
        ...
        【★レアリティ判定 (肉球ランク 1〜5)】
        そのアイテムや場所の「見つけにくさ」「希少性」をネル先生が独自判定してください。
        - **1 (★)**: どこにでもあるもの（鉛筆、ノート、石ころ、コンビニ商品、日常の風景）
        - **2 (★★)**: ちょっと嬉しいもの（きれいな花、少し珍しいお菓子、公園の遊具）
        - **3 (★★★)**: 特定の場所に行かないと見れないもの（地域の有名な公園、観光地の看板、期間限定商品、猫）
        - **4 (★★★★)**: かなり珍しいもの（歴史的建造物、有名なテーマパーク、高級なブランド品、美しい夕焼け）
        - **5 (★★★★★)**: 奇跡レベル・超レア（世界遺産、四つ葉のクローバー、野生動物、虹、めったに見れない絶景）
