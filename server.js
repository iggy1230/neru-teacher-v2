// --- server.js (完全版 v307.0: 位置情報特定プロンプト強化版) ---
...
app.post('/identify-item', async (req, res) => {
    try {
        ...
        if (location && location.lat && location.lon) {
            locationInfo = `【重要：位置情報】\n撮影場所の座標は 緯度:${location.lat}, 経度:${location.lon} です。
            まず、この座標を使ってGoogle検索を行い、この場所が具体的にどこなのか（施設名、観光地名、店舗名、市町村名）を特定してください。
            その上で、画像に写っているものを特定してください。`;
        }
